...
void call_fn(char* lib_dir, char* lib_name, char* fn_name, u_int8_t args_len, linker_value args[ARG_MAX],
  ...
  for (u_int8_t i = 0; i < args_len; i++) {
    ...
    switch (arg.type) {
      case linker_type_buffer: {
        ...
        ffi_arg_type_elements[i][1] = &ffi_type_ulong, ffi_arg_type_elements[i][2] = NULL;
  ...
  ffi_cif cif;
  ffi_status status = ffi_prep_cif(&cif, FFI_DEFAULT_ABI, args_len, &ffi_return_type, ffi_argtypes_refs);
  if (status != FFI_OK) {
